...
@section('breadcrumb', 'Stock / Kurangi Stok')

@section('content')
<div class="dms-card">
    <div style="margin-bottom: 2rem;">
        <h3 style="font-size: 1.2rem; font-weight: 600; color: var(--k-gray-800);">Kurangi Stok</h3>
        <p style="font-size: 0.85rem; color: var(--k-gray-500);">
            Kurangi stok untuk produk: <strong>{{ $product->name }}</strong>
        </p>
    </div>

    <form action="{{ route('stock.reduce', $product) }}" method="POST">
        @csrf
        ...
        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem;">
            ...
            <div class="form-group">
                <label class="form-label">Jumlah Stok Dikurangi <span style="color: var(--k-red);">*</span></label>
                <input type="number" name="quantity" class="form-control" required min="1" max="{{ $product->current_stock }}" placeholder="Masukkan jumlah stok">
                <small>Maksimal: {{ number_format($product->current_stock) }}</small>
                @error('quantity') <span style="color: var(--k-red); font-size: 0.75rem;">{{ $message }}</span> @enderror
            </div>
            
            <!-- Stok Setelah Dikurangi -->
            <div class="form-group">
                <label class="form-label">Stok Setelah Dikurangi</label>
            ...
            <div class="form-group" style="grid-column: span 2;">
                ...
                <textarea name="reason" class="form-control" rows="2" placeholder="Contoh: Rusak, kadaluarsa, digunakan untuk demo, dll">{{ old('reason') }}</textarea>
        ...
        <div style="display: flex; gap: 1rem; justify-content: flex-end; margin-top: 2rem; padding-top: 1rem; border-top: 1px solid var(--k-gray-200);">
            ...
            <button type="submit" class="dms-btn dms-btn-primary">
                <i class="bi bi-save"></i> Kurangi Stok
            </button>
        </div>
    </form>
</div>
