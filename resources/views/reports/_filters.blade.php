...
<form method="GET" style="display: flex; gap: 0.75rem; flex-wrap: wrap; align-items: flex-end; margin-bottom: 1.5rem; padding: 0.9rem; border: 1px solid #e3ebf5; border-radius: 8px; background: #f8fbff;">
    @isset($filters)
        <div style="min-width: 280px; flex: 1 1 280px;">
            <label style="{{ $labelStyle }}">Cari Produk</label>
            <input type="search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Nama produk, kategori, principal..." style="{{ $fieldStyle }}">
        </div>
    @endisset
    ...
    @isset($exportType)
        <a class="dms-btn dms-btn-outline" style="height: 44px; min-width: 132px; justify-content: center;" href="{{ route('reports.export', array_merge(['type' => $exportType], request()->only(['start_date', 'end_date', 'principal_id', 'search', 'category', 'insight', 'per_page']))) }}">
