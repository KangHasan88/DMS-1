...
@section('breadcrumb', 'Katalog / Kategori Produk / Edit')

@section('content')
<div class="dms-card">
    <div class="dms-form-header">
        <h3 class="dms-form-title">Edit Kategori Produk</h3>
        <p class="dms-form-subtitle">Edit kategori: {{ $productCategory->name }}</p>
    </div>
    ...
    <form action="{{ route('product-categories.update', $productCategory) }}" method="POST">
        ...
        <div class="dms-form-grid">
            <div class="form-group">
                ...
                <input type="text" name="name" value="{{ old('name', $productCategory->name) }}" class="form-control" required>
                @error('name') <span class="dms-error">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label class="form-label">Urutan</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', $productCategory->sort_order) }}" class="form-control" min="0">
