...
<div class="dms-card">
    <div class="dms-section-header">
        ...
        <div class="dms-toolbar-actions">
            @can('process payment')
                @if($customerPayment->status !== \App\Models\CustomerPayment::STATUS_VOID)
                    <button type="button" class="dms-btn dms-btn-outline" onclick="document.getElementById('void-customer-payment-form').classList.toggle('d-none')">
    ...
    @if($customerPayment->status === \App\Models\CustomerPayment::STATUS_VOID)
        <div class="dms-alert dms-alert-warning">
            <strong>Payment void.</strong> {{ $customerPayment->void_reason ?: 'Tanpa alasan.' }}
        </div>
    @endif

    @can('process payment')
        @if($customerPayment->status !== \App\Models\CustomerPayment::STATUS_VOID)
            <form id="void-customer-payment-form" action="{{ route('customer-payments.void', $customerPayment) }}" method="POST" class="dms-form-section d-none" style="margin-bottom: 1rem; padding: 1rem; border: 1px solid #e3ebf5; border-radius: 8px; background: #f8fbff;">
                @csrf
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 0.75rem; margin-bottom: 1rem;">
        ...
        <div style="border: 1px solid var(--k-border); border-radius: 8px; padding: 0.875rem;">
            ...
            <strong>{{ $customerPayment->chartAccount?->code ? $customerPayment->chartAccount->code . ' - ' . $customerPayment->chartAccount->name : '1110 - Kas dan Bank' }}</strong>
        </div>
        <div style="border: 1px solid var(--k-border); border-radius: 8px; padding: 0.875rem;">
            <span style="display: block; color: var(--k-gray-500); font-size: 0.75rem; margin-bottom: 0.35rem;">Status</span>
            <span class="dms-badge dms-badge-{{ $customerPayment->status_badge }}">
                {{ $customerPayment->status_label }}
            </span>
        </div>
    </div>
